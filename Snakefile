...
rule align_to_genome:
    input:
        r1="new_fastq/{sample}_R1.trimmed.fastq.gz",
        star_index="/blue/giustirodriguezp/PROJECTS/genome/Mus_musculus/Parabricks/STAR_index_100",
        fasta="/blue/giustirodriguezp/PROJECTS/genome/Mus_musculus/Star_index_119_read/GRCm38.primary_assembly.genome.fa"
    output:
        "log/star_{sample}.txt",
        "alignment/{sample}Aligned.sortedByCoord.out.bam"
    log:
        "log/parabricks_rna_{sample}.log"
    threads: 16
    resources:
        gpus=1,
        mem_mb=64000
    shell:
        r"""
        set -euo pipefail
        mkdir -p alignment log

        REFARG=""
        if [ -n "{input.fasta}" ] && [ -f "{input.fasta}" ]; then
          REFARG="--ref {input.fasta}"
        fi

        # Write BAM directly to the expected STAR-style file name
        pbrun rna_fq2bam \
          $REFARG \
          --genome-lib-dir {input.star_index} \
          --max-collapsed-junctions 5500000 \
          --in-se-fq {input.r1} \
          --out-bam {output[1]} \
          --output-dir alignment/{wildcards.sample} \
          --sjdb-overhang 99 \
          --num-gpus {resources.gpus} \
          --num-threads {threads} \
          --low-memory \
          --read-files-command zcat \
          --max-out-filter-multimap 50 \
          --min-score-filter 0.30 \
          --min-match-filter-normalized 0.30 \
          --out-filter-intron-motifs None \
          --max-intron-size 50000 \
          --min-intron-size 80 \
          --out-reads-unmapped Fastx \
          --out-sam-attributes All \
          --out-prefix {wildcards.sample} \
          &> {log}

        # Ensure expected BAM exists and produce the simple completion log
        test -s {output[1]}
        echo "STAR alignment is complete" > {output[0]}
        """
...
rule unique_mappers:
    ...
    shell:
        """
         {input[1]} view -b -q 255 -o {output} {input[0]} 
        """
...
rule all_barcodes:
    ...
    shell:
        """
         python {input[1]} -i {input[0]} -o {output}
        """
...
rule cluster_single_cells:
    ...
    shell:
        """
         ...
         cd single
         python {splitbycell} /blue/giustirodriguezp/PROJECTS/Sprite/Guttman_Ismagilov_Labs/KK_scSPRITE/scSPRITE/cluster/clusters_all_reform
